			...
			if (strlen($res) <= 0)
			{
				...
				if ($fileID)
				{
					...
					if ($file)
					{
						...
						$tmp["fileURL"] = CFile::GetPath($fileID);//CHTTP::URN2URI($APPLICATION->GetCurPageParam("mfi_mode=down&fileID=".$fileID."&cid=".$cid."&".bitrix_sessid_get(), array("mfi_mode", "fileID", "cid")));
				...
				$arFileField = CFile::MakeFileArray($fileID);
    				$arFileField['del'] = "Y";           
    				$arFileField['old_file'] = $arResult["arUser"]['PERSONAL_PHOTO'];

				$fields = array(
	    			"PERSONAL_PHOTO" => $arFileField,
        			);
