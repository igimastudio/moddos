...
class CCatalogViewedProductsComponent extends CBitrixComponent
{
	...
	protected function checkModules()
	{
		if (!Main\Loader::includeModule("iblock"))
			throw new SystemException(Loc::getMessage("CVP_IBLOCK_MODULE_NOT_INSTALLED"));
	...
	public function onPrepareComponentParams($params)
	{
		...
		if (strlen($params["BASKET_URL"]) <= 0)
			$params["BASKET_URL"] = "/personal/basket.php";

		$params["ACTION_VARIABLE"] = trim($params["ACTION_VARIABLE"]);
		if (strlen($params["ACTION_VARIABLE"]) <= 0 || !preg_match("/^[A-Za-z_][A-Za-z01-9_]*$/", $params["ACTION_VARIABLE"]))
			$params["ACTION_VARIABLE"] = "action";

		$params["PRODUCT_ID_VARIABLE"] = trim($params["PRODUCT_ID_VARIABLE"]);
		if (strlen($params["PRODUCT_ID_VARIABLE"]) <= 0 || !preg_match("/^[A-Za-z_][A-Za-z01-9_]*$/", $params["PRODUCT_ID_VARIABLE"]))
		...
		if (strlen($params["PRODUCT_QUANTITY_VARIABLE"]) <= 0 || !preg_match("/^[A-Za-z_][A-Za-z01-9_]*$/", $params["PRODUCT_QUANTITY_VARIABLE"]))
			$params["PRODUCT_QUANTITY_VARIABLE"] = "quantity";

		$params["PRODUCT_PROPS_VARIABLE"] = trim($params["PRODUCT_PROPS_VARIABLE"]);
		if (strlen($params["PRODUCT_PROPS_VARIABLE"]) <= 0 || !preg_match("/^[A-Za-z_][A-Za-z01-9_]*$/", $params["PRODUCT_PROPS_VARIABLE"]))
	...
	protected function checkSection($sectionId = -1, $sectionCode = "")
	{
		...
		if(!$bSectionFound && strlen($sectionCode) > 0)
		{
	...
	{
		$ids = array();
		if($this->arParams["SECTION_ID"] <= 0 && strlen($this->arParams["SECTION_CODE"]) == 0)	// no section specified
		{
	...
	protected function getReferences()
	{
		...
		$cached = array();
		if ($obCache->StartDataCache($this->arParams["CACHE_TIME"], $cacheId))
		{
			// Catalog Groups
	...
	{
		if (!count($this->productIdsMap) || !count($this->arParams['SHOW_PRODUCTS']))
			return array();
	...
	protected function prepareFilter()
	{
		...
		$this->filter = array(
			"ID" => count($this->productIdsMap) <= 0 ? -1 : array_values($this->productIdsMap),
			"IBLOCK_LID" => SITE_ID,
			"IBLOCK_ACTIVE" => "Y",
			"ACTIVE_DATE" => "Y",
			"ACTIVE" => "Y",
