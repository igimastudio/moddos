			...
			if(isset($_REQUEST["auth_service_id"]) && $_REQUEST["auth_service_id"] <> '' && isset($arResult["AUTH_SERVICES"][$_REQUEST["auth_service_id"]]))
			{
				...
				elseif(!$oAuthManager->Authorize($_REQUEST["auth_service_id"]))
				{
					...
					if ($ex)
                        ...
                        foreach ($arServices as $serv_id => $service)
                        {
                            
                            $cl = new $service["CLASS"];
                            if(is_callable(array($cl, "getUrl")))
                                $arResult["AUTH_SERVICES"][$serv_id]["URL"] = htmlspecialcharsbx(CUtil::JSEscape(call_user_func_array(array($cl, "getUrl"), array($arResult))));
                            else
                                $arResult["AUTH_SERVICES"][$serv_id]["URL"] = htmlspecialcharsbx(CUtil::JSEscape($GLOBALS['APPLICATION']->GetCurPageParam('auth_service_id='.$serv_id.'&check_key='.$_SESSION["UNIQUE_KEY"].(isset($arParams['BACKURL']) ? "&backurl=".urlencode($arParams['BACKURL']) : ''), array("logout", "auth_service_error", "auth_service_id", "current_fieldset"))));
