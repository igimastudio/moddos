	...
	if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit"]) || !empty($_REQUEST["iblock_apply"])))
	{
		...
		if (empty($arResult["ERRORS"]))
		{
			...
			$arUpdateValues["PROPERTY_VALUES"] = $arUpdatePropertyValues;
                        $arUpdateValues["PROPERTY_VALUES"]["RATING"] = $arUpdateValues["RATING"];
                        $arUpdateValues["PROPERTY_VALUES"]["RECOMEND"] = $arUpdateValues["RECOMEND"];
			...
			else
			{
				if (strlen($SEF_URL) > 0)
					...
					$sRedirectUrl = $APPLICATION->GetCurPageParam("edit=Y&CODE=".$arParams["ID"], array("edit", "CODE"), $get_index_page=false);
                        ...
                        $arEventFields["LINK"] = "/bitrix/admin/iblock_element_edit.php?IBLOCK_ID=".$arParams["IBLOCK_ID"]."&type=".$arParams["IBLOCK_TYPE"]."&ID=".$arParams["ID"]."";
                        $event = new CEvent;
                        $event->SendImmediate("NEW_REVIEW", SITE_ID, $arEventFields);
