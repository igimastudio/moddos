...
if($this->StartResultCache(false, array($arrFilter, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $arNavigation)))
{
	...
	elseif($arParams["SECTION_ID"] > 0)
	{
		...
		$rsSection = CIBlockSection::GetList(Array(), $arFilter, true, $arSelect);
	...
	elseif(strlen($arParams["SECTION_CODE"]) > 0)
	{
        ...
        if (!empty($arParams["ELEMENT_CODE"]))
        {
            ...
            $res = CIBlockElement::GetList(Array("SORT"=>"ASC"), $arFilter, false, false, Array("ID","CODE","IBLOCK_ID"));
            while($ar_fields = $res->GetNext())
                $arNavParams["nElementID"]=$ar_fields["ID"];
        }
        if (!empty($arParams["ELEMENT_ID"]))
            $arNavParams["nElementID"]=$arParams["ELEMENT_ID"];
	if(!$bSectionFound)
	{
		$this->AbortResultCache();
		ShowError(GetMessage("CATALOG_SECTION_NOT_FOUND"));
	...
	while($obElement = $rsElements->GetNextElement())
	{
		...
		$arItem['ACTIVE_TO'] = $arItem['DATE_ACTIVE_TO'];
                if ($arParams["ELEMENT_CODE"] == $arItem["CODE"])
                    $arItem["CURRENT_ITEM"] = "Y";
		if($arResult["ID"])
