...
if ($USER->IsAuthorized() || $arParams["ALLOW_AUTO_REGISTER"] == "Y")
{
	...
	{
            if ($arParams["PRODUCT_ID"] > 0 && CModule::IncludeModule("catalog"))
            {
                $pid = Add2BasketByProductID(
                    $arParams["PRODUCT_ID"],
                    1,
                    array(), 
                    array()
		...
		$arSelFields = array("ID", "CALLBACK_FUNC", "MODULE", "PRODUCT_ID", "QUANTITY", "DELAY",
			...
			"NOTES", "DISCOUNT_PRICE", "PRODUCT_PROVIDER_CLASS", "DIMENSIONS", "TYPE", "SET_PARENT_ID", "DETAIL_PAGE_URL"
                ...
                $arFilters = array("FUSER_ID" => CSaleBasket::GetBasketUserID(),
						"LID" => SITE_ID,
						"ORDER_ID" => "NULL");
		...
		while ($arItem = $dbBasketItems->GetNext())
		{
			...
			$arResult["PRICE_WITHOUT_DISCOUNT"] = SaleFormatCurrency($arResult["ORDER_PRICE"] + $DISCOUNT_PRICE_ALL, $allCurrency);
                        $arResult['DISCOUNT_PRICE_ALL']= $DISCOUNT_PRICE_ALL+$arResult['DISCOUNT_PRICE'];
                        $arResult['DISCOUNT_PRICE_ALL_FORMATED']= SaleFormatCurrency($arResult['DISCOUNT_PRICE_ALL'], $arResult["BASE_LANG_CURRENCY"]);
		...
		if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["confirmorder"]) && ($arParams["DELIVERY_NO_SESSION"] == "N" || check_bitrix_sessid()))
		{
			...
			while ($arOrderProps = $dbOrderProps->GetNext())
			{
				...
				//{               
					...
					if ($bErrorField && $arParams["STEP"] == 2)
				...
				$arResult["ORDER_PROP"]["USER_PROPS_Y"][$arProperties["ID"]] = $arProperties;
                        ...
                        {   
                            if($arProperties["TYPE"] != "LOCATION")
                            {
                                ...
                                {
                                    $arResult["ORDER_PROP"]["USER_PROPS_N"][$arProperties["CODE"]] = $arProperties;
                                    unset($arResult["ORDER_PROP"]["USER_PROPS_N"][$arProperties["ID"]]);
                                }
                                else  
                                    $arResult["ORDER_PROP"]["USER_PROPS_N"][$arProperties["ID"]] = $arProperties;
                            }
                            else
                                $arResult["ORDER_PROP"]["LOCATION"][$arProperties["ID"]] = $arProperties;
                        }
		...
		$arResult['DELIVERY_PRICE_FORMATED'] = SaleFormatCurrency($arOrderDat["DELIVERY_PRICE"], $arResult["BASE_LANG_CURRENCY"]);
                if (is_array($arOrderDat['BASKET_ITEMS']))
		...
		$orderTotalSum = $arResult["ORDER_PRICE"] + $arResult["DELIVERY_PRICE"] + $arResult["TAX_PRICE"];
		...
		if($arUserResult["CONFIRM_ORDER"] == "Y" && empty($arResult["ERROR"]))
		{
			if(!$USER->IsAuthorized() && $arParams["ALLOW_AUTO_REGISTER"] == "Y")
			{
				if(strlen($arUserResult["USER_EMAIL"]) > 0)
				{
					...
					{
                                                if ($_REQUEST["NEWS_SUB"] == "Y" && strlen($NEW_EMAIL)>0)
                                                        IgimaTools :: AddUserSub($arAuthResult,$NEW_EMAIL,4);
						...
						if ($USER->IsAuthorized())
						{
							if($arParams["SEND_NEW_USER_NOTIFY"] == "Y")
                                                        {
								$event = new CEvent;
                                                                $event->SendImmediate("REGISTER_OK", SITE_ID, $arEventFields);
                                                        }
						}
						else
						{
					...
					$arResult["ERROR"][] = GetMessage("STOF_ERROR_EMAIL");
                        ...
                        {
                                if ($_REQUEST["NEWS_SUB"] == "Y" && strlen($USER->GetEmail())>0)
                                        IgimaTools :: AddUserSub($USER->GetID(),$USER->GetEmail(),4);
			...
			if($USER->IsAuthorized() && empty($arResult["ERROR"]))
			{
				...
				if (empty($arResult["ERROR"]))
				{
					$arResult["REDIRECT_URL"] = $arParams["PATH_TO_ORDER"]."?ORDER_ID=".urlencode(urlencode($arOrder["ACCOUNT_NUMBER"]));
		...
		$arResult["USER_VALS"] = $arUserResult;
                ...
                $arResult["ORDER_PROP"]["USER_PROPS_N"]["COUNTRY"] = $arLocs["COUNTRY_NAME"];
                $arResult["ORDER_PROP"]["USER_PROPS_N"]["CITY"] = $arLocs["CITY_NAME"];
	}
	else
	{
		$arOrder = false;
		...
		if ($arOrder && $arOrder["USER_ID"] == IntVal($USER->GetID()))
		{
			...
			{
                            $delid = explode(":",$arOrder["DELIVERY_ID"]);
                            if (count($delid)>1)
                            {
                                $dbResult = CSaleDeliveryHandler::GetBySID($delid[0]);
                                if ($arDeliv = $dbResult->GetNext())
                                    $arResult["DELIVERY"] = $arDeliv;
                            }
                            else
                            {
                                $arResult["DELIVERY"] = CSaleDelivery::GetByID($arOrder["DELIVERY_ID"]);
                            }
                        }
                        $arStatus = CSaleStatus::GetByID($arOrder["STATUS_ID"]);
                        $arOrder["STATUS_NAME"] = $arStatus["NAME"];
                        $arOrder["PRICE_FORMATED"] = SaleFormatCurrency($arOrder["PRICE"], $arOrder["CURRENCY"]);
		...
		$arSelFields = array("ID", "CALLBACK_FUNC", "MODULE", "PRODUCT_ID", "QUANTITY", "DELAY",
			"CAN_BUY", "PRICE", "WEIGHT", "NAME", "CURRENCY", "CATALOG_XML_ID", "VAT_RATE",
			"NOTES", "DISCOUNT_PRICE", "PRODUCT_PROVIDER_CLASS", "DIMENSIONS", "TYPE", "SET_PARENT_ID",
                        "DETAIL_PAGE_URL"
		);
                $arFilters = array("FUSER_ID" => CSaleBasket::GetBasketUserID(),
						"LID" => SITE_ID,
		...
		foreach ($arResult["BASKET_ITEMS"] as &$arResultItem)
		{
                        $arResultItem["SUM"] = SaleFormatCurrency($arResultItem["PRICE"] * $arResultItem["QUANTITY"], $arResult["BASE_LANG_CURRENCY"]);
			$productId = $arResultItem["PRODUCT_ID"];
			$arParent = CCatalogSku::GetProductInfo($productId);
			if (intval($arProductData[$productId]["PREVIEW_PICTURE"]) <= 0
					&& intval($arProductData[$productId]["DETAIL_PICTURE"]) <= 0
