...
function getOrderPropFormated($arProperties, $arResult, &$arUserResult, &$arDeleteFieldLocation = array())
{
	...
	elseif ($arProperties["TYPE"] == "TEXT")
	{
		if (strlen($curVal) <= 0)
		{
			...
			elseif ($arProperties["FIELD_ID"] == "ORDER_PROP_3")
			{
				//$arProperties["VALUE"] = $USER->GetFullName();
				$rsUser = CUser::GetByID($USER->GetID());
				if ($arUser = $rsUser->Fetch())
				{
					$arProperties["VALUE"] = $arUser["LAST_NAME"]." ".$arUser["NAME"]." ".$arUser["SECOND_NAME"];
				}
				
			}
			elseif ($arProperties["FIELD_ID"] == "ORDER_PROP_6")
			{
				//$arProperties["VALUE"] = $USER->GetFullName();
				$rsUser = CUser::GetByID($USER->GetID());
				if ($arUser = $rsUser->Fetch())
				{
					$arProperties["VALUE"] = $arUser['PERSONAL_ZIP'];
				}
				
			}
			elseif ($arProperties["FIELD_ID"] == "ORDER_PROP_phone")
			{
				//$arProperties["VALUE"] = $USER->GetFullName();
				$rsUser = CUser::GetByID($USER->GetID());
				if ($arUser = $rsUser->Fetch())
				{
				$arProperties["WORK_PHONE"] = $arUser["WORK_PHONE"];
				$arProperties["VALUE"] = $arUser['PERSONAL_MOBILE'];
				}
			}
			elseif ($arProperties["FIELD_ID"] == "ORDER_PROP_street")
			{
				//$arProperties["VALUE"] = $USER->GetFullName();
				$rsUser = CUser::GetByID($USER->GetID());
				if ($arUser = $rsUser->Fetch())
				{
				$arProperties["VALUE"] = $arUser['PERSONAL_STREET'];
				}
			}
			elseif ($arProperties["FIELD_ID"] == "ORDER_PROP_house")
			{
				//$arProperties["VALUE"] = $USER->GetFullName();
				$rsUser = CUser::GetByID($USER->GetID());
				if ($arUser = $rsUser->Fetch())
				{
				$arProperties["VALUE"] = $arUser["UF_HOUSE"];
				}
			}
			elseif ($arProperties["FIELD_ID"] == "ORDER_PROP_flat")
			{
				//$arProperties["VALUE"] = $USER->GetFullName();
				$rsUser = CUser::GetByID($USER->GetID());
				if ($arUser = $rsUser->Fetch())
				{
				$arProperties["VALUE"] = $arUser["UF_APARTAMENTS"];
				}
			}
		}
		else
